...
//! Core node.
extern crate qtumcore_rpc;

use qtumcore_rpc::{qtum, Auth, Client, Error, RpcApi};
...
fn main_result() -> Result<(), Error> {
    ...
    let bitcoin_block: qtum::Block = rpc.get_by_id(&best_block_hash)?;
    println!("best block hash by `get`: {}", bitcoin_block.header.prev_blockhash);
    let bitcoin_tx: qtum::Transaction = rpc.get_by_id(&bitcoin_block.txdata[0].txid())?;
